...
| `ocr-toc-*` | OCR 目录缓存状态：独立提示与校正/重识入口落点（ocr-toc-notice）、校正目录条目决议（pdf-ocr-toc-cache）、探测/识别/保存三取一同步锁（ocr-toc-op）；分级评估见 shared/reader/ocr-toc-assess，可用性门控见 pdf-ocr-toc-gate |

...
| \`.md\` / \`.markdown\` / \`.txt\` and other plain text | Prefer your **normal workspace file read/write**. Do not use Inkdown tools just to read the current file. Exception: to find where a word occurs in the currently open .md (including unsaved edits), use \`inkdown_inspect_content\` or \`inkdown_read(scope=search)\` instead of reading the file from the workspace. Other workspace files are not searchable yet (no rg). |

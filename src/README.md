...
主进程代码在 [`electron/`](../electron/README.md)；跨进程契约在 `packages/contracts/`（`@inkdown/contracts`，原 `shared/`）；阅读器纯逻辑在 `packages/reader-core/`（`@inkdown/reader-core`）。细则见根目录 [AGENTS.md](../AGENTS.md)。

...
主进程代码在 [`electron/`](../electron/README.md)；跨进程契约在 `packages/contracts/`（`@inkdown/contracts`，原 `shared/`）；阅读器纯逻辑在 `packages/reader-core/`（`@inkdown/reader-core`）；PDF 归一/映射在 `packages/pdf/`（`@inkdown/pdf`，页码归一/分类映射/整档拼装）；OCR 纯逻辑在 `packages/ocr-core/`（`@inkdown/ocr-core`，目录/水印/页质量判断及 pdf-bytes 与缓存 ports）。细则见根目录 [AGENTS.md](../AGENTS.md)。

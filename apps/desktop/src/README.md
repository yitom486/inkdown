...
主进程代码在 [`electron/`](../electron/README.md)；跨进程契约在 `packages/contracts/`（`@inkdown/contracts`，原 `shared/`）；ACP 纯逻辑在 `packages/acp/`（`@inkdown/acp`：stdio JSON-RPC 传输/认证决策链/会话能力与 registry/终端缓冲/MCP RPC 与工具表；`session-open` 留守主进程，渲染端零直接引用）；阅读器纯逻辑在 `packages/reader-core/`（`@inkdown/reader-core`）；PDF 归一/映射在 `packages/pdf/`（`@inkdown/pdf`，页码归一/分类映射/整档拼装）；OCR 纯逻辑在 `packages/ocr-core/`（`@inkdown/ocr-core`，目录/水印/页质量判断及 pdf-bytes 与缓存 ports，外部运行时 pin 见 `inspector-pins`）；标注在 `packages/annotations/`（`@inkdown/annotations`：标记合并/纯核/Anki 构建/Flashcard 模型与复习，另含标记提议 `mark-proposal` 与章级建议 `chapter-mark-plan`）；在线文档目录抽取/站点谓词在 `packages/web-doc/`（`@inkdown/web-doc`）。细则见根目录 [AGENTS.md](../../AGENTS.md)。

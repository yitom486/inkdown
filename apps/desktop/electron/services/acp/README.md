...
| 认证 | `codex-auth-preflight.ts` 留守（类型单一源 `@inkdown/contracts` 别名再导出）；`packages/acp/src/auth/connect-auth-gate.ts` / `packages/acp/src/auth/connect-auth-decision.ts` / `packages/acp/src/auth/auth-method-order.ts` / `packages/acp/src/auth/codex-provider-home.ts` 已迁 `@inkdown/acp` | 连接前探活、authMethods 顺序、`~/.codex`；自定义供应商隔离 CODEX_HOME 的 config.toml 生成 |

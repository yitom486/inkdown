...
import type { AcpProviderConfig, AcpProviderSavePayload, AcpProviderStatus, AppError } from '@inkdown/contracts'
import { err, ok, type Result } from '@inkdown/contracts'

/** 主进程侧完整配置（含明文 Key）；渲染端只见 AcpProviderStatus */
...
export async function readStoredAcpProvider(dir = app.getPath('userData')): Promise<StoredAcpProvider | null> {
...
/** 供 UI 的脱敏状态（Key 不回传） */
export async function getAcpProviderStatus(dir = app.getPath('userData')): Promise<AcpProviderStatus> {
...
export async function saveAcpProvider(
  ...
  const file = providerFilePath(dir)
  // Key 留空 = 沿用已存 Key（Key 不回传渲染端，编辑时无法回填）
  const apiKey =
...
export async function clearAcpProvider(dir = app.getPath('userData')): Promise<Result<void, AppError>> {
  await rm(providerFilePath(dir), { force: true })
  return ok(undefined)
}

/** 隔离 CODEX_HOME 目录（spawn 前由 acp-client 落 config.toml） */

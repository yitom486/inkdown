...
| `web-doc-service.ts` | 在线文档抓取与目录发现入口（网络边界留守：SSRF 策略与重定向校验见 `web-doc/url-policy.ts`；目录抽取/站点谓词已迁 `@inkdown/web-doc`，本树 `web-doc/` 仅留 `url-policy`/`site-registry`/`e2e-fixture`） |
...
| `ai-session-service.ts` | AI 会话指针（一书一会话，`inkdown.db ai_sessions` 行存取；轮转决策在调用方） |

## 结构化索引（SQLite 罗盘，单书一库）

| 目录 | 功能 |
|------|------|
| `app-db/` | 全局库 adapter（`userData/inkdown.db` 跨书数据）：`schema` 建表 + migrate（`user_version`，v1 建 `quiz_sessions` + `quiz_questions` + 索引，v2 加 `ai_sessions` 一书一会话指针）；`open-app-db` 单库打开 + 句柄缓存 |
...
| 目录 | 功能 |
|------|------|
| `sync/` | WebDAV 存储 adapter 与同步管理器整体留守；`mergers/` 仅留 `progress`/`quiz`，`marks` 合并已迁 `@inkdown/annotations` |

# 更新日志

记录 [Inkdown](README.md) 各版本**主要功能更新**。版本号与 Git tag 对齐。

**发版时**：日常把要点写在 `[未发布]` → 执行 `bun run release`（默认 **patch** 自动 +1）→ 确认后 `bun run release:push` 推送 tag 触发 GitHub Release。不必手算版本号；大改动用 `bun run release -- minor` / `major`。不必罗列每条 commit 或小修复。

## [未发布]

- ### Agent
  - **agy 改走 bunx 直调**：删掉自建 managed 安装器（顺带解除仅 Windows 限制），与 codex 同构；装不上的 EINVAL 根因已修
  - **bunx 三家锁定最新**：agy / codex / deepseek / claude 模板统一加 `@latest`，每次冷启动直取 registry 最新（24 小时缓存坑已避开）
  - **agy 首次安装有提示**：连接徽标旁明示约 100MB 下载等待，不再干转圈

- ### 工程
  - **发版脚本修坑**：`release:push` 改为只推不断版，发版后再推不会被空节拦住、也不会误连发一版

---

## [0.4.2] - 2026-09-22

- ### Agent（多运行时）
  - **一口气支持 8 个 Agent**：ChatGPT、Claude、Gemini、Copilot、OpenCode、Cursor、DeepSeek、agy，面板内一键切换，各自历史与会话隔离，名字不再带技术后缀
  - **复用本机登录**：直接沿用各家 CLI 自己的登录态（ChatGPT 的 `~/.codex`、Cursor/OpenCode 等同理），登过就直连；没登录弹对应指引
  - **agy 自动安装与更新**：首次连接自动下载桥组件（约 100MB，会提示等待），之后每次连接自动检查 npm 最新版
  - **模型/思考档/Context/快速开关**：输入栏直接切换（能力以各 Agent 下发的为准）；快速模式首次默认关闭并记住偏好
  - **会话恢复不叠字**：重连回放自动压制，漏网气泡自动收尾冻结；恢复的隐形上下文不再灌进聊天记录
  - **连不上说人话**：缺 key、缺 CLI、子进程秒退等失败直接给出原因与动作，不再只有裸错误码

- ### 协议底层
  - **直迁官方 ACP SDK**：删掉手搓 JSON-RPC 传输（旧代码归档为教学快照），与 Zed 同源；握手声明参数化模型选择，对齐新版 Zed 行为

---

## [0.4.1] - 2026-09-21

- ### 阅读器

  - **知识卡片统一文档纵序**：右侧卡片轨、悬浮窗卡片流、导出全部按"先章节后文中位置"排列，同章内不再按创建时间错位
  - **卡片↔正文双向跳转打通**：点卡定位正文（PDF/在线文档支持页内定位，不再只跳页首），点正文标记反向定位卡片，悬浮窗卡片同样可跳
  - **EPUB 标记渲染修复**：下划线与页边圆点同源定位，圆点随滚动跟随，跨章漏画收敛
  - **底栏翻页收进正文列**：不再横跨卡片轨；上一单元/当前单元/下一单元逻辑不变
  - **顶栏删除**：文件/编辑/视图/帮助整行移除，入口收进资源管理器 ⋯ 菜单；新建窗口改 Ctrl+Shift+N
  - **停靠面板可左右拉伸**，悬浮窗可拖拽改大小；聊天历史分页（初看末 5 条，上滑懒加载），打开即沉底无滚动动画

- ### 数据

  - **卡片锚点绑定落库**：book.db v7 新增 `anchor_key` 抽取列与索引，一卡一段、一段多卡，升级自动回填

---

## [0.4.0] - 2026-09-17

- ### Agent

  - **自定义模型供应商**：可配置 OpenAI 兼容 API 的 Base URL、API Key、模型与协议，支持 DeepSeek 等服务；配置使用隔离的 Codex 运行目录，不影响本机 `~/.codex` 订阅登录
  - **上下文更精简**：环境说明只在会话首次发送，后续仅发送短工具索引与动态阅读快照；目录整理使用独立的专用 Agent 会话与 `toc_*` 工具
  - **MCP 工具并发提示**：只读阅读工具增加标准只读/幂等注解，服务端支持并发处理独立的只读工具调用；批注与目录写入仍采用安全的顺序/批量提交

- ### 阅读器与在线文档

  - **结构整理**：阅读器、Agent、布局与 ACP 内部模块按产品域重新归类，降低查找和维护成本，不改变用户功能
  - **TanStack Query 缓存**：测验历史、同步状态、罗盘书籍信息与 OCR 组件状态接入缓存与失效机制，减少重复 IPC 读取

---

## [0.3.1] - 2026-09-11

- **首次 OCR 安装**：补全检测模型被截断的 SHA256，哈希失败不再空转重试，报错会标明文件名与字节数

---

## [0.3.0] - 2026-09-10

### 扫描 PDF：打开就能划，全书建议先入库

- **当前页静默识别**：无文字层的页面打开后自动认这一页，不必先等全书入库即可划词、划重点、写批注；失败不弹框，工具栏「识别本页」仍可手动重试
- **扫描书请优先建立罗盘**：全书搜索、按章阅读、内容取证都走索引；未入库时这些能力会明确提示，而不会悄悄整书 OCR。纯文字 PDF 建索引会跳过 OCR 运行时
- **导入中不叠跑**：全书识别进行中，单页与目录识别会等待完成或取消，避免内存打满；已完成的页翻到即可划，页词缓存按页合并
- **可重新建立罗盘**：删旧库后全书再识别（划重点 / 批注保留）；目录与库不一致时只做本地目录更新，不重新 OCR

### 目录：书签只是参考，自建课级目录为准

- **识别印刷目录**：即使 PDF 自带书签（页标签、版权页那种），扫描 / 混合文档也可探测目录页并用 OCR 构建章节目录；保存后侧栏、切章、入库都以自建目录为准
- **清除才退回书签**：丢掉目录缓存后才回到内置书签；未保存的校正草稿不会拿去入库
- **探测与校正**：可自动查找目录页范围、选择候选、AI 整理与页码偏移推算；导入中不可再开目录识别

### OCR 引擎与划词质量

- **识别引擎换代**：单页与目录改走 pdf-inspector 选择性识别（PP-OCRv6），中文与表格更好；Tesseract 及语言包已删除，中英文字库在模型内
- **运行时按需分发**：PDFium / ONNX Runtime / 模型首次识别时下载验签（约 110MB，一次性），之后离线可用
- **划词更干净**：斜向水印戳印从划词层滤掉；可预览并应用正文水印清洗。划词后聚焦输入框，点击聊天不会清掉选区；Ctrl+C 复制当前选区（空输入框不会抢走），工具栏「复制」后黄条仍保留

### Agent

- **已入库只读罗盘**：viewport / 当前页 / 章 / 搜索 / 取证都只读库，缺页报错，不再静默 OCR；「识别本页」只给人看词层，不写回索引
- **未入库扫描书**：可问选区和当前页；全书搜索与取证会请你先建立罗盘
- **快照更准**：阅读位置变化会重贴快照，PDF 带上当前页码

---

## [0.2.9] - 2026-09-06

### 工程优化（体积 · 性能 · 安全 · 可维护）

- **首屏体积 -77%**：PDF / EPUB / MOBI / 在线文档阅读器改为按需加载，主包 `index-*.js` 由 5.6MB 降至 1.31MB；Mermaid / KaTeX 确认早已独立分包
- **阅读后端统一**：EPUB / MOBI / AZW3 改走 foliate 同一套解析渲染（删 epubjs 与 mobi-parser 双栈），划重点 / 批注 / 进度 / 导出链路不变
- **同步与供应链更稳**：WebDAV 单次 15s 超时 + 指数退避重试；OCR 运行时下载验 SHA256（缺 manifest 的老 Release 放行告警）；划线墓碑 90 天 TTL 自动回收
- **安全收口**：在线文档抓取拦截内网 / 保留地址（含各类 IP 变体）与重定向跳板；`inkdown://` 深度链接仅接受 `/open`；渲染器内非应用 origin 跳转一律转系统浏览器；新增 CSP 强制策略（脚本同源 + Tab 运行时 hash 放行，hash 有单测锁定）
- **工程门禁**：新增 `bun run lint:docs`（子目录 README 与文件清单一致性）与 `bun run check:bundle`（体积预算 + 主进程打包白名单），均接入 CI；图标生成改纯 Bun/Node 跨平台实现；postinstall 兼容无 Node 纯 Bun 环境
- **测试**：单测 676 通过（144 文件）；e2e 新增 QuickOpen、WebDAV 本地同步、XSS 真浏览器回归、阅读器冒烟（自研最小 PDF / EPUB / Mermaid / 公式 fixture），12 条全绿
- **纵深安全**：渲染进程开启沙盒（`sandbox: true`，preload 改 CJS 打包并显式 external），全量 e2e 验证通过
- **Agent 读 PDF 更准**：新增 pdf-inspector WASM 结构化解析（专用 Worker，阅读顺序 + 紧凑 Markdown），Agent 当前页/整书正文优先使用，任何失败静默回退 pdf.js；渲染/选区/批注/OCR 链路不动；CSP 增 `wasm-unsafe-eval`（仅自带 WASM）；`out/` 总量门禁同步放宽至 30MB
- **混合 PDF 识别**：扫描判断抽样由前 3 页扩展为前 3 + 中间 + 末尾，混合文档自动放行单页 OCR（扫描横幅与后台预识别仍只跟纯扫描）
- **打包**：macOS 仅发 Apple Silicon 版（上游 Rust 解析器无 x64 预编译）

### WebDAV 轻量多端云同步系统 (Serverless Sync)

- **零服务器私有化架构 (Local-First)**：
  - 基于 Node 原生 `fetch` 实现了标准 RFC 4918 WebDAV 客户端，零外部库依赖负担
  - 支持 **坚果云（Jianguoyun）**、**Nextcloud** 与各类 **自建 NAS（群晖 Synology、极空间、绿联、Alist 等）**
  - 内置坚果云专属预设与应用密码生成引导；支持自建 NAS 自签名 HTTPS 证书信任开关
- **轻量核心元数据双向无损归并 (Smart Merge)**：
  - **划线与书签**：UUID 并集归并 + 毫秒级时间戳仲裁 + 墓碑软删除索引（Tombstones），彻底防止对端已删除划线复活
  - **阅读进度**：EPUB、MOBI、PDF 与在线文档各格式基于最新更新时间戳自动仲裁，跨电脑无缝续读
  - **AI 测验档案**：基于测验 ID 幂等去重追加，保持时间线连续
- **设置中心交互控制台**：
  - 提供连接实时测速（ms）与远程目录权限一键验证
  - 提供手动「立即同步」按钮（带加载动画与增量统计通知）
  - 支持应用启动时自动拉取同步（带 3 秒缓冲），本地阅读进度变动时节流持久化

### 闪卡

- **纯文本契约**：复习用的 `front`/`back` 不再内嵌 Anki HTML；导出 Anki 时再包一层，避免复习界面露出原始标签
- **本章闪卡计数**：修复按 TOC `key` 误匹配导致「本章」恒为空；改为按阅读位置的 `matchKey` 定位，并显示本章/全书可复习张数

### Agent 与阅读器

- **流式输出更顺滑**：正文增量 32ms 合并提交、聊天记录低频持久化；展示层按输入速度自适应匀速揭示（rAF 追赶、保留约 150ms 余量），稳定段落冻结复用、未闭合代码块跳过高亮，完成时平滑排空再切精确渲染。

- **Agent 批量标记提议卡**：修复 `inkdown_propose_mark` 的 MCP 结果未进入 ACP 会话更新时，批量高亮 / 批注建议卡不显示的问题；兼容快照结果早于工具事件到达的时序，并避免已采用的旧回合状态串入新卡片。
- **EPUB 稳定性**：切换书籍、快速卸载或开发热更新时，取消尚未执行的 epub.js 渲染启动队列，避免 `Rendition.start()` 访问已销毁 book 导致阅读器报错。

### 系统与运行

- **数据目录与并发沙箱**：主进程优先遵循命令行传入的 `--user-data-dir` 独立目录，彻底解决多端并发、E2E 测试与开发/生产版本同时运行时的 Chromium 缓存与单实例互斥锁冲突。

### 阅读器拆分（无行为变化）

- **划选工具条共享**：三阅读器复制 / 批注 / 高亮 / 加入对话 / 问 Agent 收敛为 `useReaderSelectionActions`
- **导出菜单共享**：笔记与 Anki 导出收敛为 `useReaderExportMenu`（toc / 当前章由各家传入）
- **PDF 单页 OCR 独立**：页缓存 / 识别去重 / 统一正文读取抽为 `usePdfPageOcr`；顺手修文档切换时去重表不清、旧文档在途任务串入新文档的问题

---

### 发布流水线修复

- **Release 检出补 submodule**：`release.yml` 质量与打包两处 checkout 加 `submodules: recursive`，否则 foliate 子模块为空导致单测挂、打包与 Release 跳过

---

---

## [0.2.8] - 2026-09-03

### AI 伴读考官与知识测验系统

- **多题组卷与批量判卷**：
  - 支持单重点与章节多重点深度组卷，默认出 3 道启发题，支持 `1题速测 / 3题标准(默认) / 5题深度` 一键切换
  - 试卷式题卡导航栏，实时显示作答状态标记、字数统计与 `Ctrl+Enter` 一键提交整卷
  - 真实大模型单次高效批量判卷，输出各题得分、采分点比对清单（✅ 准确涵盖 / ⚠️ 遗漏深化要点）、导师精炼点评与整卷平均分
- **两级轻量分类与知识档案**：
  - 一级物理分类：自动关联归档书籍与章节（`bookTitle` + `chapterTitle`）
  - 二级认知标签：大模型自动标注认知维度标签（`概念认知` / `逻辑因果` / `批判反思` / `细节辨析` 等）
  - 本地追加型存储：基于 `.inkdown/quiz-records.jsonl`，完全本地私有化保存
- **成绩回放中心 (Quiz History & Replay)**：
  - 随时回放历史测验，支持多题试卷逐题复盘、重温读者手写原始答案与 AI 判卷分析，支持一键定位原书划线
- **考官会话生命周期与分流隔离**：
  - 考官专属独立副会话，空闲超时放宽至 2 小时，避免频繁换会话；支持手动一键「重启会话」
  - 主进程与渲染进程双层物理分流拦截，出题与判卷过程 100% 隔离，绝不污染右侧主对话面板

### 阅读器与批注

- **本章重点综合卷**：高亮批注工具栏支持针对单条重点或整章所有划线快速发起综合测验
- **视觉优化**：隐藏题卡导航条 Windows 原生横向滚动条，保留滚轮与触控平滑滑动

---

## [0.2.7] - 2026-09-02

### PDF OCR（扫描版）

- **按需组件**：打包版不内置 Tesseract；首次使用从 GitHub Release 下载 OCR 运行时；开发环境沿用 devDependency
- **语言包**：`chi_sim` / `eng` 按需下载；设置页管理组件与缓存
- **阅读链路**：单页识别与缓存、合成 textLayer、划词划重点、Agent 读页与后台预识别
- **目录**：视觉目录 OCR、页码偏移、侧栏校正与保存；嵌入书签 `dest` 解析增强
- **设置**：识别精度档位、按书/全局清理 OCR 缓存

### Agent

- **Bun 检测与安装**：连接前检测 `bunx` 依赖；未安装时明确提示与一键安装（按系统脚本）

---

## [0.2.6] - 2026-09-01

### 在线文档

- **人民日报电子版**：阅读模式解析版面与文章页；本版新闻目录；正文链接应用内跳转
- 修复跳转后显示原始网页、阅读区黑屏、正文内链接无法点击

### Agent

- **数学公式**：聊天气泡 KaTeX 渲染；支持 `\[...\]`、`\(...\)`、裸括号行内写法；流式时补全未闭合定界符
- **流式贴底滚动**：仅在底部附近自动跟随；上滑暂停并显示「回到底部」；批注 AI 区行为一致
- **工具失败说明**：聊天气泡内折叠展示；展开为业务语句（不直接甩原始报错）
- **关闭面板保留会话**：点 × 仅收起面板；再打开回到原对话（新建请用 +）

### 工作区

- **侧栏在线文档**：打开本地文件夹后，侧栏底部仍可输入 URL 打开网页文档

---

## [0.2.5] - 2026-09-01

### 网页模式 Agent

- **无本地工作区也可聊**：打开在线文档即可连接 Agent（ACP 使用应用沙箱 cwd）；附加本地文件仍需先打开文件夹
- **左侧栏对齐**：无文件夹时展示当前页与最近 URL，不再只催「打开文件夹」
- **本页大纲**：侧栏底部「目录」解析当前网页正文 h1–h6（与顶栏整站 TOC 分离）

### Agent 标记助手（P1–P3）

- **会话内提议卡**：Agent 批注 / 高亮提议在聊天气泡内确认，不再弹全局 Dialog
- **无划词定位**：按摘录模糊匹配定位；统一 MCP 工具 `inkdown_propose_mark`（单条 / 批量）
- **章级划重点**：`inkdown_suggest_chapters` 建议章节 → 用户点选 → 批量提议；支持勾选采用
- **MCP 瘦身**：读正文 / 列表 / 提议参数化；`inkdown_get_selection` 独立保留
- **P4 加固**：PDF 相邻页摘录定位与缩放后 live 重算高亮；在线文档折叠空白 / 跳过行号匹配；提议失败提供「打开该章 / 去划词」快捷动作

---

## [0.2.3] - 2026-08-31

### 产品

- 应用正式定名为 **Inkdown**（窗口标题、欢迎页、安装包与关于对话框）
- **应用内更新**：打包版启动后检查 [GitHub Releases](https://github.com/yitom486/inkdown/releases)，可下载并重启安装；关于 / 设置中也可手动检查
- 关于对话框补充产品说明与仓库链接

> **从 v0.2.x 升级**：安装包 appId 已变更，旧版「轻量阅读器」无法原地自动升级，请下载新安装包覆盖或并行安装；此后同 Inkdown 安装包之间可走应用内更新。

### 在线文档

- 欢迎页输入 URL 或打开 React 官方文档；主进程抓取 HTML、阅读模式渲染
- 与电子书同一套阅读壳：目录、底栏步进、划词高亮 / 批注、标记侧栏、笔记导出
- 顶栏**地址栏**切换页面；**代码块一键复制**（含 react.dev 等站点语言识别）
- Agent 可通过 MCP 读取当前页、视口、目录章节与阅读标记

### 批注与阅读

- **批注 AI 助手**：选区对话、一键「写成批注」、手写优先与 AI 润色；独立会话，与主 Agent 面板互不干扰
- 修复 PDF 远距离跳转（目录 / 书签）后黑屏不渲染
- 在线文档划词高亮：文本锚定与空白归一化，减轻布局变化后错位

### 开发

- 在线文档 E2E：本地 fixture 冒烟（不依赖外网）
- Release 产物包含 `latest.yml`，供 electron-updater 使用

---

## [0.2.2] - 2026-08-30

- **划重点**：选区标记重点，多色高亮可编辑
- **阅读笔记导出**：书签与批注导出 Markdown，可按全书或当前章节筛选
- 标记侧栏按类型筛选、按章节分组；PDF 划选与批注体验改进
- Windows 安装包支持自选安装目录

---

## [0.2.1] - 2026-08-30

- **Agent 面板**：流式对话、多会话历史、权限审批与工具调用预览
- **Agent 阅读集成**：可读当前章节 / 视口 / 选区，书签与批注 MCP 工具
- 资源管理器右键菜单（新建、重命名、删除、复制等）
- Agent 输入区支持拖入文件与粘贴图片；选区可「问 Agent」
- PDF 中文显示与清晰度修复

---

## [0.2.0] - 2026-08-29

### Agent 助手

- 接入 ACP 流式对话、Codex 登录认证、权限审批与终端工具调用
- 支持本地多会话历史、恢复会话、活动时间线、工具调用与 Mermaid 渲染
- Agent 面板支持调整宽度；文件可拖入或粘贴为上下文附件

### 工作区与阅读体验

- 资源管理器新增 VS Code 风格右键菜单及新建、重命名、删除、复制等文件操作
- 优化 Markdown 导出 PDF；修复 PDF 中文缺字、画布清晰度与未保存状态误判

---

## [0.1.1] - 2026-08-29

- 提供 **Windows / macOS / Linux** 三平台安装包
- 统一阅读器章节导航（底栏步进、侧栏完整目录）
- 修复 MOBI / AZW3 目录跳转与章内导航问题

---

## [0.1.0] - 2026-08-29

首个公开发布版本。

- **Markdown 编辑**：分屏预览、目录同步、GFM / 公式 / Mermaid、查找替换、导出 HTML·PDF、主题与设置
- **多格式阅读**：PDF、EPUB、MOBI、AZW3 / AZW；连续滚动、章节导航、阅读进度记忆
- **书签与批注**：高亮、笔记、侧栏统一管理

---

[未发布]: https://github.com/yitom486/inkdown/compare/v0.4.2...HEAD
[0.4.2]: https://github.com/yitom486/inkdown/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/yitom486/inkdown/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/yitom486/inkdown/compare/v0.3.1...v0.4.0
[0.3.1]: https://github.com/yitom486/inkdown/compare/v0.3.0...v0.3.1
[0.3.0]: https://github.com/yitom486/inkdown/compare/v0.2.9...v0.3.0
[0.2.9]: https://github.com/yitom486/inkdown/compare/v0.2.8...v0.2.9
[0.2.8]: https://github.com/yitom486/inkdown/compare/v0.2.7...v0.2.8
[0.2.7]: https://github.com/yitom486/inkdown/compare/v0.2.6...v0.2.7
[0.2.6]: https://github.com/yitom486/inkdown/compare/v0.2.5...v0.2.6
[0.2.5]: https://github.com/yitom486/inkdown/compare/v0.2.3...v0.2.5
[0.2.3]: https://github.com/yitom486/inkdown/compare/v0.2.2...v0.2.3
[0.2.2]: https://github.com/yitom486/inkdown/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/yitom486/inkdown/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/yitom486/inkdown/compare/v0.1.1...v0.2.0
[0.1.1]: https://github.com/yitom486/inkdown/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/yitom486/inkdown/releases/tag/v0.1.0

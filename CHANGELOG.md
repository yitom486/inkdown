...
- **识别引擎换代**：单页与目录 OCR 改走 pdf-inspector 选择性识别（PP-OCRv6），中文准确率与表格还原提升；Tesseract 及语言包机制删除，中英文字库内置于模型
- **运行时按需分发**：PDFium / ONNX Runtime / 模型首次识别时下载验签（约 110MB，一次性），后续离线可用；设置页可查看进度与取消
